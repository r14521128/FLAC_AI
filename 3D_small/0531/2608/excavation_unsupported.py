...
# 小模型 sub_2608：無支撐逐步開挖
#
# 功能：
#   1) 監測三個岩體點：頂拱、左側壁、右側壁。
#   2) 只在收斂後輸出最大塑性厚度。
#
# Run in FLAC3D IPython Console:
#   exec(open("D:/FLAC_AI/3D_small/0531/2608/excavation_unsupported.py").read())
# ============================================================
...
# 路徑與模型設定
# ============================================================
BASE_DIR = r"D:\FLAC_AI\3D_small\0531\2608"
PARENT_DIR = r"D:\FLAC_AI\3D_small\0531"
MODEL_ID = "sub_2608"

# 材料參數掃描：由環境變數 FLAC_COMBO_ID 指定（如 'c01'）。
#   有設定 -> restore sub_2608_initial_state_<combo>
#            （reader 已賦該組材料 + IDW 應力 + 切割邊界），
#            所有輸出檔名加 _<combo> 後綴。
#   未設定 -> 維持原行為（restore sub_2608_build）。
COMBO_ID = os.environ.get("FLAC_COMBO_ID", "").strip()
RUN_TAG = MODEL_ID + ("_" + COMBO_ID if COMBO_ID else "")
...
if COMBO_ID:
    INITIAL_STATE_CANDIDATES = [
        os.path.join(BASE_DIR, "sub_2608_initial_state_" + COMBO_ID),
    ]
else:
    # 優先讀子資料夾內的 build save；若尚未搬入，退回讀 0531 根目錄。
    INITIAL_STATE_CANDIDATES = [
        os.path.join(BASE_DIR, "sub_2608_build"),
        os.path.join(PARENT_DIR, "sub_2608_build"),
    ]
...
print("RUN_TAG = {0}".format(RUN_TAG))
print("restore = {0}".format(INITIAL_STATE_NAME))
...
# CSV 輸出
# ============================================================
csv_path = os.path.join(OUTPUT_DIR, "unsupported_excavation_monitor_{0}.csv".format(RUN_TAG))
...
for x_group in excavation_groups:
    ...
    if step % CHECKPOINT_INTERVAL == 0:
        it.command("model save '{0}/{1}_chk_{2:04d}'".format(
            OUTPUT_DIR.replace("\\", "/"), RUN_TAG, step))
