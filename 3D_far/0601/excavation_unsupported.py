...
#   exec(open("D:/FLAC_AI/3D_far/0601/excavation_unsupported.py").read())
# ============================================================
...
# 路徑設定
# ============================================================
base_dir = r"D:\FLAC_AI\3D_far\0601"
...
initial_state_name = "D:/FLAC_AI/3D_far/0601/initial_state_0601"
...
# ============================================================
# 用途：
#   FLAC3D 的 zone.mech.ratio 是 FISH 內的全域平均力比 (average)。
#   這個腳本需要在 Python 迴圈中讀取它，判斷是否已收斂。
#   做法是找一個穩定存在的參考 zone，把 ratio_avg 暫存到 zone.extra(10)，
...
# converged：
#   1 = 達到目標力比
#   cap_ratio=<ratio_avg> = 撞到 SOLVE_CAP 時的實際 ratio_avg
#   空白 = cycle 過程點
monitor_header = [
...
R_done = False
step_pair = 0
crossed_monitors = set()   # 已被開挖面越過的監測斷面 x（每斷面只觸發一次詳細記錄）
...
while (L_idx < len(left_groups) and not L_done) or \
    ...
    # 監測斷面步：開挖面「首次越過」某監測斷面 x 的那一步（每斷面只觸發一次）
    is_monitor_step = False
    for mx in MONITOR_X:
        if mx in crossed_monitors:
            continue
        if (L_group and max(L_group) >= mx) or (R_group and min(R_group) <= mx):
            crossed_monitors.add(mx)
    ...
    if is_monitor_step:
        ...
        if not ok:
            print("  [WARN] step_pair={0} monitor NOT converged to 1e-5 "
                  "(cap {1}, ratio={2:.6e})".format(
                      step_pair, SOLVE_CAP, ratio_now))
    elif step_fine:
        # fine 步：直接收斂 1e-5，只記最終那一筆（含花費 cycle）
        used, ok = solve_to_ratio(RATIO_FINE)
        ratio_now = get_ratio()
        conv_status = convergence_status(ok, ratio_now)

        write_all_rows(step_pair, "conv_1e-5", used,
                       L_group, R_group, RATIO_FINE, conv_status,
                       calc_plastic=True)
